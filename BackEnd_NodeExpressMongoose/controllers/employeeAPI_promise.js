...
const InsModel = require('../models/instrument_schema');
...
// And Receive value from request body and Store value within the Object
                   const empobj = new EmpModel({
                                 ...
                                 empsession: req.body.empsession,
                                 empaddress: req.body.empaddress,
								 empheight: req.body.empheight,
								 empweight: req.body.empweight,
                                 });//CLOSE EmpModel
...
router.post('/add', (req, res) => 
    ...
    res.status(200).send('DOCUMENT INSERED IN MONGODB DATABASE' +'<br\>'+ inserteddocument);
                         ...
                         .catch(err =>{
    res.status(500).send({ message: err.message || 'Error in Instrument Save '})
                               });//CLOSE CATCH
                            }//CLOSE CALLBACK FUNCTION BODY
                            );//CLOSE POST METHOD


router.put('/updateins', (req, res) => 
            {
    
  InsModel.findOneAndUpdate({"insname" : req.body.insname}, 
                              { $set: {"insno":req.body.insno
							  } }, { new: true })
          .then(getupdateddocument => {
            if(getupdateddocument != null)
               res.status(200).send('DOCUMENT UPDATED ' + getupdateddocument);  
            else
               res.status(404).send('INVALID Instrument '+ req.params.insname);
          }) // CLOSE THEN
          .catch(err => {
  return res.status(500).send({message: "DB Problem..Error in UPDATE with id " + req.params.insname });
          }) // CLOSE CATCH
                            } //CLOSE CALLBACK FUNCTION Line No 108
                            ); //CLOSE PUT METHOD Line No 107
...
router.delete('/delete/:insnameid', (req, res) =>
            {
  InsModel.findOneAndRemove({"insname" : req.params.insnameid})
          .then( deleteddocument => { 
            if(deleteddocument != null)
            {  
  res.status(200).send('DOCUMENT DELETED successfully!' + deleteddocument);
            ...
            {
  res.status(404).send('INVALID Ins Name '+ req.params.insname); 
            }
          }) //CLOSE THEN
          .catch( err => {
 return res.status(500).send({message: "DB Problem..Error in Delete with name " + req.params.insname });          
          })//CLOSE CATCH
             }//CLOSE CALLBACK FUNCTION BODY
            ); //CLOSE Delete METHOD
...
router.get('/ins/', (req, res) => 
    ...
    res.status(200).send(getalldocumentsfrommongodb);
                          ...
                          .catch(err =>{
    res.status(500).send({ message: err.message || 'Error in Fetch Instrument '})
                          });//CLOSE CATCH
                } //CLOSE CALLBACK FUNCTION BODY Line 110      
          );
...
router.get('/search/ins/:insnameid', (req, res) => 
            {
      InsModel.find({"insname" : req.params.insnameid})
            .then(getsearchdocument => {
              if(getsearchdocument.length >0) 
              {
                res.send(getsearchdocument);
              ...
              {
  return res.status(404).send({message: "Note not found with name " + req.params.insname });
              }
          }) //CLOSE THEN
            .catch( err => {
  return res.status(500).send({message: "DB Problem..Error in Retriving with id " + req.params.insname });           
            })//CLOSE CATCH
          }//CLOSE CALLBACK FUNCTION BODY Line 88
         );//CLOSE GET METHOD Line 87 
